...
if(isset($_REQUEST['eid']))
{
  ...
  <script>
    ...
    }).then(()=>{ window.location='canceled-bookings.php'; });
  </script>";
}
...
if(isset($_REQUEST['aeid']))
{
  ...
  <script>
    Swal.fire({
      icon:'success',
      title:'Confirmed',
      text:'Booking Successfully Confirmed',
      timer:1500,
      showConfirmButton:false
    }).then(()=>{ window.location='confirmed-bookings.php'; });
  </script>";
}
...
<head>
	...
	<title>ProModel Car Rental | New Bookings</title>
...
<body>
    ...
    <tr>
      ...
      <td>
        <?php 
        if($result->Status==0){ echo htmlentities('Not Confirmed yet'); }
        else if($result->Status==1){ echo htmlentities('Confirmed'); }
        else { echo htmlentities('Cancelled'); }
        ?>
      </td>
    ...
    <tr>	
      <td style="text-align:center" colspan="4">
        ...
        <a href="bookig-details.php?aeid=<?php echo htmlentities($result->id);?>"
           onclick="confirmBooking(event,this.href)"
        ...
        <a href="bookig-details.php?eid=<?php echo htmlentities($result->id);?>"
           onclick="cancelBooking(event,this.href)"
